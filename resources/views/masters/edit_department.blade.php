...
<div id="main-wrapper">
    <div class="panel panel-white">
        <div class="panel-body"> 
            <form method="POST" enctype="multipart/form-data" action="{{ route('update-department') }}" class="formSave">
                @csrf
                <input type="hidden" name="id" value="{{ @$departments->id }}" >
                <div class="row">
                    <div class="col-md-6">
                        <label for="name">Department Name <span style="color: red">*</span></label>
                        <input type="text" class="form-control" name="name" id="name" placeholder="Enter department name" autocomplete="off" value="{{ @$departments->name }}" required>
                    </div>
                    <div class="col-md-6">
                        ...
                        <select name="is_active" id="is_active" class="form-control select2" required>
                            ...
                            <option value="1" @if(!empty($departments->is_active) && $departments->is_active == 1) selected @endif>Active</option>
                            <option value="2" @if(!empty($departments->is_active) && $departments->is_active == 2) selected @endif>In-Active</option>
                        </select>
                    </div>
                    <div class="col-md-6">
