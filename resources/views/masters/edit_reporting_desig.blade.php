...
<div id="main-wrapper">
    <div class="panel panel-white">
        <div class="panel-body">
            <form method="POST" enctype="multipart/form-data" action="{{ route('save-project') }}" class="formSave">
                @csrf
                ...
                <div class="row">
                    <div class="col-md-3">
                        ...
                        <select name="dept_id" id="dept_id" class="form-control select2" required>
                            ...
                            <option value="{{ $department->id }}" @if(!empty($reporting->dept_id) && $reporting->dept_id == $department->id) selected @endif>{{ $department->name }}</option>
                            @endforeach
                            @endif
                        </select>
                    </div>
                    <div class="col-md-3">
                        ...
                        <select name="desig_id" id="desig_id" class="form-control select2" required>
                            ...
                            <option value="{{ $designation->id }}" @if(!empty($reporting->desig_id) && $reporting->desig_id == $designation->id) selected @endif>{{ $designation->name }}</option>
                            @endforeach
                            @endif
                        </select>
                    </div>
                    <div class="col-md-3">
                        ...
                        <select name="reporting_id" id="reporting_id" class="form-control select2" required>
                            ...
                            @if(isset($designations) && count($designations) > 0)
                            @foreach($designations as $designation)
                            <option value="{{ $designation->id }}" @if(!empty($reporting->reporting_id) && $reporting->reporting_id == $designation->id) selected @endif>{{ $designation->name }}</option>
                            @endforeach
                            @endif
                        </select>
                    </div>
                    <div class="col-md-3">
                        ...
                        <select name="is_active" id="is_active" class="form-control select2" required>
                            <option value=""> --Select-- </option>
                            <option value="1" @if(!empty($reporting->is_active) && $reporting->is_active == 1) selected @endif>Active</option>
                            <option value="2" @if(!empty($reporting->is_active) && $reporting->is_active == 2) selected @endif>In-Active</option>
                        </select>
                    </div>
                </div>
